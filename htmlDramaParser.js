...
async.waterfall([
	function (callback) {
		request(programUrl, function (err, res, body) {
			...
			for (var i = 70; i < cateListLen-1; i++) {
				var stitle = $(categoryList[i]).text().split("(");
				var title = stitle[0].replace(/[^가-힣\w-,:]/g,"");
		...
		request(url, function (err, res, body) {
			...
			if (version=="토도우") {
				...
				for (var i = 0; i < a.length-1; i++) {
					...
					var shref = href.split("view/")[1]
					if (shref){
						var tudouId = shref.match(/[\w-]{11}(?=\/)/g);
	...
	function (err, title, epnum, date, version, id, img) {
		...
		Episode.findOneAndUpdate({title: title, epnum: parseInt(epnum)}, {$set: {title: title, epnum: epnum, date: date, outline: outline}}, {upsert:true}, function (err, episode) {
